...
export async function getAuraStats() {
  return request(`/aura/stats`);
}

export async function getAuraLeaderboard(sortBy = "aura", limit = 10) {
  return request(`/aura/leaderboard?sort_by=${encodeURIComponent(sortBy)}&limit=${encodeURIComponent(limit)}`);
}

export async function getAuraWallet(address) {
  return request(`/aura/wallet/${encodeURIComponent(address)}`);
}

export async function syncAura() {
  return request(`/aura/sync`, { method: "POST" });
}
...
export async function saveWallet(walletAddress) {
  ...
  const url = `${API_BASE}/auth/wallet`;
  const res = await fetch(url, {
    method: "POST",
    headers: {
