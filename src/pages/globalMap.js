...
// Search autocomplete
let searchLabelsData = null; // Данные из labels.json для поиска по странам
const searchCache = new Map(); // Кеш результатов поиска
...
export function destroyGlobalMap() {
  ...
  publicTooltipNameEl = null;
  publicTooltipCountEl = null;
  ...
  panel.innerHTML = `
    ...
    <div class="location-search" id="location-search">
      <div class="form-group">
        <label for="location-search-input">Quick Search</label>
        <input type="text" id="location-search-input" placeholder="Search country or city..." autocomplete="off" />
      </div>
      <div class="location-search__results hidden" id="location-search-results"></div>
    </div>
    
    <div class="location-panel__divider"></div>
...
async function loadSearchLabels() {
  if (searchLabelsData) return searchLabelsData;
  try {
    const res = await fetch('/data/labels.json');
    searchLabelsData = await res.json();
    return searchLabelsData;
  } catch (e) {
    console.warn('Failed to load labels for search:', e);
    return [];
  }
}
...
async function searchLocations(query) {
  ...
  // 1. Поиск по странам (из labels.json)
  const labels = await loadSearchLabels();
  for (const label of labels) {
    ...
    if (results.length >= 8) break; // Лимит по странам
  }
  
  // 2. Поиск по городам через Nominatim (только если < 8 результатов и запрос >= 3 символов)
  if (results.length < 10 && normalizedQuery.length >= 3) {
    try {
      ...
      for (const item of data) {
        // Пропускаем если это страна (уже есть в результатах)
        if (item.type === 'country' || item.class === 'boundary') continue;
...
function setupSearchAutocomplete() {
  searchInputEl = locationPanel?.querySelector('#location-search-input');
  searchResultsEl = locationPanel?.querySelector('#location-search-results');
  
  if (!searchInputEl || !searchResultsEl) return;
  
  // Загружаем данные заранее
  loadSearchLabels();
  
  // Input handler с debounce
  ...
  searchInputEl.addEventListener('keydown', (e) => {
    ...
    const items = searchResultsEl.querySelectorAll('.location-search__item');
    if (!items.length) return;
...
function renderSearchResults(results) {
  ...
  if (!results.length) {
    searchResultsEl.innerHTML = '<div class="location-search__empty">No results found</div>';
  ...
  const html = results.map((r, i) => {
    const icon = r.type === 'country' ? '🌍' : '📍';
    const display = r.type === 'city' ? r.displayName : r.name;
    const subtitle = r.type === 'country' ? 'Country' : 'City';
    
    return `
      <div class="location-search__item" data-index="${i}" data-type="${r.type}" data-name="${r.name}" data-lat="${r.lat}" data-lon="${r.lon}" data-country="${r.country || r.name}">
        <span class="location-search__icon">${icon}</span>
        <div class="location-search__text">
          <div class="location-search__name">${display}</div>
          <div class="location-search__type">${subtitle}</div>
        </div>
      </div>
    `;
  }).join('');
  ...
  // Click handlers
  searchResultsEl.querySelectorAll('.location-search__item').forEach(item => {
...
function updateActiveSearchItem(items) {
  items.forEach((item, i) => {
    item.classList.toggle('is-active', i === activeSearchIndex);
  });
}
...
function handleSearchSelect(item) {
  ...
  // Показываем tooltip со статистикой для страны
  if (type === 'country') {
    // Находим страну в countriesIndex и показываем tooltip
    const countryKey = _canonCountryKey(name);
    const count = publicCountryCounts.get(countryKey) || 0;
    
    // Устанавливаем координаты для tooltip
    const rect = canvasHost?.getBoundingClientRect();
    if (rect) {
      publicPointerX = rect.left + rect.width / 2;
      publicPointerY = rect.top + rect.height / 2;
    }
    
    showPublicHoverTooltipAt(null, { name, count });
    
    // Устанавливаем highlight
    if (countriesIndex) {
      const found = countriesIndex.find(c => 
        c.name.toLowerCase() === name.toLowerCase() || 
        _canonCountryKey(c.name) === countryKey
      );
      if (found) {
        setCountryHighlight(found);
      }
    }
  } else {
    // Для города — заполняем форму
