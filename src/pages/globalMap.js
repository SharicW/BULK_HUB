...
export async function initGlobalMap(target) {
  ...
  try {
    ...
    // Ждём один кадр чтобы DOM layout завершился и контейнер получил размеры
    await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    
    // Проверяем что контейнер имеет реальные размеры
    await waitForValidSize(stageEl, 3000);

    await init();
    
    // Принудительный resize после инициализации для гарантии корректных размеров
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      if (Date.now() - startTime > timeoutMs) {
        ...
        resolve({ width: window.innerWidth * 0.7, height: window.innerHeight * 0.6 });
        return;
      }
...
function getStageSize() {
  ...
  if (stageEl && stageEl.offsetWidth > 100 && stageEl.offsetHeight > 100) {
    return { width: stageEl.offsetWidth, height: stageEl.offsetHeight };
  }
  
  // Последний fallback: используем процент от окна
  const fallbackWidth = Math.max(320, window.innerWidth * 0.7);
  const fallbackHeight = Math.max(320, window.innerHeight * 0.6);
  console.warn('Globe: using fallback size', fallbackWidth, fallbackHeight);
