...
// Если нет - пробуем исправить через resize
...
function scheduleRenderCheck() {
  ...
  renderCheckTimeout = setTimeout(() => {
    ...
    // Проверяем что canvas имеет реальные размеры
    const canvas = renderer.domElement;
    if (!canvas || canvas.width === 0 || canvas.height === 0) {
      console.warn('Globe: render check failed - canvas has zero size, forcing resize');
      handleResize();
      
      // Ещё одна проверка через секунду
      setTimeout(() => {
        if (isActive && renderer) {
          const canvas2 = renderer.domElement;
          if (!canvas2 || canvas2.width === 0 || canvas2.height === 0) {
            console.warn('Globe: second render check failed, another resize attempt');
            handleResize();
          }
        }
      }, 1000);
